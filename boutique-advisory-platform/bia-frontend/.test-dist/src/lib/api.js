...
let authMeLastResponse = null;
const apiDebug = process.env.NEXT_PUBLIC_API_DEBUG === 'true';
function __resetApiTestState() {
    csrfToken = null;
...
async function ensureCsrfToken() {
    ...
    const fetchUrl = `${exports.API_URL}/api/csrf-token?t=${Date.now()}`;
    if (apiDebug) {
...
async function apiRequest(endpoint, options = {}) {
    const url = `${exports.API_URL}${endpoint}`;
    ...
    const shouldShareAuthMe = method === 'GET' &&
        endpoint.split('?')[0] === '/api/auth/me';
    const shouldRetryTransient = endpoint.split('?')[0] === '/api/auth/login';
    const transientStatuses = new Set([502, 503, 504]);
    const executeFetch = () => fetch(requestUrl, {
        ...options,
        headers,
        cache: 'no-store',
        credentials: 'include',
