...
router.get('/', authorize('wallet.read'), async (req: AuthenticatedRequest, res: Response) => {
    try {
...
router.get('/history', authorize('wallet.read'), async (req: AuthenticatedRequest, res: Response) => {
    try {
        const userId = req.user?.id;
