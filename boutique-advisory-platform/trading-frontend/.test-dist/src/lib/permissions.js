...
exports.createPermissionHelpers = createPermissionHelpers;
/**
 ...
 */
function hasPermission(userRole, permission, isOwner = false) {
    if (!userRole)
        return false;
    const normalizedRole = (0, roles_1.normalizeRole)(userRole);
    if (!normalizedRole)
        return false;
 ...
 */
function canPerformAction(userRole, resource, action, isOwner = false) {
    return hasPermission(userRole, `${resource}.${action}`, isOwner);
}
/**
 * Create permission helpers for a user
 */
function createPermissionHelpers(user) {
    const normalizedRole = (0, roles_1.normalizeRole)(user?.role);
    const role = (normalizedRole || undefined);
    const check = (permission, isOwner = false) => hasPermission(role, permission, isOwner);
    const canPerform = (resource, action, isOwner = false) => canPerformAction(role, resource, action, isOwner);
    return {
        // General checks
        hasPermission: check,
        canPerform,
