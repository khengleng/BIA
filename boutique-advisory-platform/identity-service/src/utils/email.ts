...
export async function sendVerificationEmail(to: string, verificationToken: string, frontendUrl?: string) {
  const verificationUrl = `${normalizeFrontendUrl(frontendUrl)}/auth/verify-email?token=${encodeURIComponent(verificationToken)}`;
...
export async function sendPasswordResetEmail(to: string, resetToken: string, frontendUrl?: string) {
  const resetUrl = `${normalizeFrontendUrl(frontendUrl)}/auth/reset-password?token=${encodeURIComponent(resetToken)}`;
