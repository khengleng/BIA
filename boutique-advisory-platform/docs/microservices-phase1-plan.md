...
1. **Monorepo Railway upload hardening**
   - Added `.railwayignore` to reduce payload size and avoid Cloudflare 413 on `railway up`.
2. **Canary-ready deployment script**
   - Added `scripts/deploy-backend-canary.sh`.
   - Script performs: clean-state guard, branch sync (`work` -> `main`), backend deploy.
